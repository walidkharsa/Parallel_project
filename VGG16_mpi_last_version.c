...
void read_weights(char *in_file, int lvls) {
	...
	if (rank==0){
		iin=fopen(in_file, "r");
		if(iin==NULL){
			printf("File %s is not available",in_file);
			MPI_Abort(MPI_COMM_WORLD,1);
			}
				}

	int file_av=(rank==0 && iin !=NULL)? 1: 0;
	MPI_Bcast(&file_av,1,MPI_INT,0, MPI_COMM_WORLD);	
	if(file_av==0){
		MPI_Finalize();
		exit(1);
		}

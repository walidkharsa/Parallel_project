...
#include <stdlib.h>     // For malloc, free, and other general utilities
...
#include <math.h>       // For math functions like exp, pow
...
#pragma omp parallel for private(j) reduction(+:sum) shared(zeropad, kernel, out, size)
	for (i = 0; i < size; i++) {
		for (j = 0; j < size; j++) {
